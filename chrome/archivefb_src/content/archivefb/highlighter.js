var archivefbHighlighter = {
	...
	{
		var idx = document.getElementById("archivefbHighlighter").getAttribute("color");
		if (idx < 1 || idx > 4)
			idx = 4;
		document.getElementById("archivefbHighlighter" + idx).setAttribute("checked", "true");
		for ( idx = 4; idx > 0; idx-- )
		{
			var cssText = archivefbUtils.getPref("highlighter.style." + idx, this.PRESET_STYLES[idx]);
			this.decorateElement(document.getElementById("archivefbHighlighter" + idx), cssText);
		}
	},
	...
	set : function(aWindow, aSelection, aNodeName, aAttributes)
	{
		for ( var r = 0; r < aSelection.rangeCount; ++r ) {
			...
			{
				archivefbUtils.alert("ERROR: Can't attach link across tags."); return;
			}
