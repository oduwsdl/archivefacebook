...
var gMultiBookManager = {
	...
	init: function() {
		...
		var data = archivefbMultiBookUI.initFile();
		var currentPath = archivefbUtils.getPref("data.path");
		...
		if (this._activeItemChanged) {
			gMultiBookTreeView._data.forEach(function(item) {
				if (item[kActiveCol]) {
					archivefbUtils.setPref("data.title", item[kNameCol]);
					archivefbUtils.setPref("data.path", item[kPathCol]);
				}
			});
		}
		var content = "";
		...
		archivefbUtils.writeFile(archivefbMultiBookUI.file, content, "UTF-8");
	...
	edit: function() {
		...
		var ret = { value: item };
		window.openDialog(
			"chrome://archivefb/content/mbEdit.xul", "", "chrome,centerscreen,modal",
			ret
		);
