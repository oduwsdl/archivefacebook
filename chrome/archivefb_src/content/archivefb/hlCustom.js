...
var hlCustomizer = {
	...
	init: function() 
	{
		...
		gPrefName   = "archivefb.highlighter.style." + gColorIndex;
		var prefVal = null;
		try {
			prefVal = gPrefBranch.getComplexValue(gPrefName, Ci.nsISupportsString).data;
		}
		catch (ex) {
			prefVal = archivefbHighlighter.PRESET_STYLES[gColorIndex];
		}
	...
	rotatePreset: function()
	{
		...
		gPreviewUI.setAttribute("style", archivefbHighlighter.PRESET_STYLES[this._presetIndex]);
		this.syncFromPreview();
	},
